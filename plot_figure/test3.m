...
for i = 1
    ...
    load(['ELM_Data_Season_11years_match_MODIS/Match_MOD_11years_ELMBC_TP_snowshape_1_iBC_eDUST_' seasonname '.mat']);
        ...
        deltaAlbedos_all(isnan(deltaAlbedos_all)) = 0;
...
%     snowAlbedos_TP(fsnos_TP<=0.01) = nan;
    ...
    deltaAlbedos_TP(isnan(deltaAlbedos_TP)) = 0;
...
% %% snow cover
% figure;
print(gcf, '-dtiff', '-r300', 'spatial_map/elm_modis_fsno_comparisonss.tif')
...
subplot('position', [0.35 0.49 0.27 0.4])
%histogram(FSA_shp2(:) - FSA_shp1(:), [-40:1:40])
plot_global_map(lats, lons, deltaAlbedos_annual_MODSCAG, 0, 0.1, 'MODSCAG', 0, 0,'(b)')
...
% %% snow cover
% figure;
print(gcf, '-dtiff', '-r300', 'spatial_map/elm_modis_sar_comparisonss.tif')
