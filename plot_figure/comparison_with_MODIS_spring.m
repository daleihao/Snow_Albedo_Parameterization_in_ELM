...
%% get MODISACG data
files = dir('MODIS/spring/MODSCAG_*.mat');
num = size(files, 1);
...
for i = 1:num
    ...
    load(['MODIS/spring/' files(i).name]);    
...
%% get MODIS SPIRES
files = dir('MODIS/spring/SPIRES_*v4.mat');
num = size(files, 1);
...
% %% snow cover
% figure;
 print(gcf, '-dsvg', '-r300', 'spatial_map/spring_comparison.svg')

%% plot
load('elevations_TP.mat');
filters = IsIndusArea & SnowCovers_annual_SPIRES>0 & elevations>1500;
...
save('boxplot_R/spring_SPIRES_ELM_v4.mat','data');

filters = ~isnan(SnowCovers_annual_MODSCAG) & SnowCovers_annual_MODSCAG>0 & elevations>1500;
...
save('boxplot_R/spring_MODSCAG_ELM.mat','data')