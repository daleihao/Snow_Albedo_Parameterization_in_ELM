...
for i = 1:num
    ...
    if(year_i <=2000 | year_i>2010 | ~(month_i>8 & month_i<12))
        continue;
    end
...
plot_global_map(lats, lons, deltaAlbedos_annual_ELM, 0, 0.1, '', 1, 1)
...
plot_global_map(lats, lons, deltaAlbedos_annual_MODSCAG, 0, 0.1, '', 1, 0)
...
plot_global_map(lats, lons, deltaAlbedos_annual_SPIRES, 0, 0.1, '', 1, 0)
hcb = colorbar;
hcb.Title.String = "Unitless";
x = hcb.Position;
x(1) = 0.93;
...
% %% snow cover
% figure;
print(gcf, '-dsvg', '-r300', 'spatial_map/spatial_autumn_comparison.svg')
