...
deltaAlbedos_annual_ELM(SnowCovers_annual_ELM<=0.0) = nan;
%% get MODISACG data
...
for i = 1:num
    ...
    if(year_i <=2000 | year_i>2010 | ~(month_i>5 & month_i<9))
        continue;
    end
...
deltaAlbedos_annual_MODSCAG(SnowCovers_annual_MODSCAG<=0) = nan;
...
deltaAlbedos_annual_SPIRES(SnowCovers_annual_SPIRES<=0) = nan;
...
ylabel({'Snow cover fraction',''},'fontsize',10,'fontweight','bold')

subplot('position', [0.07 0.08 0.27 0.4])
plot_global_map(lats, lons, deltaAlbedos_annual_ELM, 0, 0.1, '', 1, 1,'(d)')

ylabel({'Snow albedo reduction',''},'fontsize',10,'fontweight','bold')

% MODSCAG
subplot('position', [0.35 0.49 0.27 0.4])
%histogram(FSA_shp2(:) - FSA_shp1(:), [-40:1:40])
plot_global_map(lats, lons, SnowCovers_annual_MODSCAG, 0, 1, 'MODSCAG', 0, 0,'(b)')


subplot('position', [0.35 0.08 0.27 0.4])
plot_global_map(lats, lons, deltaAlbedos_annual_MODSCAG, 0, 0.1, '', 1, 0,'(e)')
...
plot_global_map(lats, lons, deltaAlbedos_annual_SPIRES, 0, 0.1, '', 1, 0,'(f)')
hcb = colorbar;
hcb.Title.String = "Unitless";
x = hcb.Position;
x(1) = 0.93;
...
% %% snow cover
% figure;
print(gcf, '-dtiff', '-r300', 'spatial_map/elm_modis_summer_comparison.tif')
...
data = [elevation_band SnowCovers_filter GrainSizes_filter deltaAlbedos_filter SnowAlbedos_filter];
save('boxplot_R/revise_spring_SPIRES_ELM_v4.mat','data');

filters = ~isnan(SnowCovers_annual_MODSCAG) & SnowCovers_annual_MODSCAG>0 & SnowCovers_annual_ELM>0 & elevations>1500;
...
save('boxplot_R/revise_spring_MODSCAG_ELM.mat','data')
