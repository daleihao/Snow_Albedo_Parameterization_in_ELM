...
{
all_data_import <- readMat(paste0('all_data',Season,'.mat'));

all_datas <- as.data.frame(all_data_import['SurfacealbedoData'])  
colnames(all_datas) <- c('elevation','snow_cover', 'shape1', 'shape2', 'shape3', 'shape4', 'BC', 'dust','BC4','dust4')

# surface albedo
all_data <- all_datas %>% 
  mutate(snow_cover = factor(SnowalbedoData)) %>%
...
plot_1 <- all_data %>%
  ...
  #scale_fill_manual() +
  scale_fill_brewer(palette = "Dark2", labels = c("Spheriod - Sphere", "Hexagonal Plate - Sphere","Koch Snowflake - Sphere")) +
...
plot_2 <- all_data %>%
  ggplot(aes(x =snow_cover , y = value, fill = variable)) + 
  ylab(expression(bold(Net~solar~radiation~"(W/"*m^{2}*")"))) + 
  xlab('Snow cover fraction')  +
  #stat_boxplot(geom ='errorbar', width = 0.6, size = 0.5, show.legend = FALSE) +
  ...
  theme(plot.title = element_text(face="bold", color="black",size=11, angle=0),
        ...
        plot.margin = unit(c(1, 0, 0, 0), "cm")) +
  #scale_fill_manual() +
  scale_fill_brewer(palette = "Dark2", labels = c("Spheriod", "Hexagonal Plate","Koch Snowflake")) +
  scale_x_discrete(labels = c('0.0-0.2','0.2-0.4','0.4-0.6','0.6-0.8','0.8~1.0')) + 
  ylim(c(-30,0))
...
plot_4 <- all_data %>%
  ggplot(aes(x =snow_cover , y = value, fill = variable)) + 
  ylab(expression(bold(Latent~heat~flux~"(W/"*m^{2}*")"))) + 
  xlab('Snow cover fraction') +
  #stat_boxplot(geom ='errorbar', width = 0.6, size = 0.5, show.legend = FALSE) +
  geom_boxplot(size = 0.3, show.legend = F, width = 0.6, outlier.shape = 1,outlier.size = 0.1, outlier.alpha = 0, position = position_dodge(width = 0.75)) + 
  #geom_jitter(trim = FALSE) +
...
plot_5 <- all_data %>%
  ggplot(aes(x =snow_cover , y = value, fill = variable)) + 
  ylab(expression(bold(Sensible~heat~flux~"(W/"*m^{2}*")"))) + 
  xlab('Snow cover fraction')  +
  #stat_boxplot(geom ='errorbar', width = 0.6, size = 0.5, show.legend = FALSE) +
  geom_boxplot(size = 0.3, show.legend = F, width = 0.6, outlier.shape = 1,outlier.size = 0.1, outlier.alpha = 0, position = position_dodge(width = 0.75)) + 
  #geom_jitter(trim = FALSE) +
...
ggsave(paste0(Season,"_Snow_shape_comparison_TP.tiff"), plot = plot_figure, width = 18, height = 15, units = "cm", dpi = 300, limitsize = FALSE, compression = "lzw")
...
plot_1 <- all_data %>%
  ...
  theme(plot.title = element_text(face="bold", color="black",size=11, angle=0),
        ...
        plot.margin = unit(c(1, 0, 0, 0), "cm")) +
  #scale_fill_manual() +
  scale_fill_brewer(palette = "Dark2", labels = c("Sphere: BC", "Sphere: Dust","Koch Snowflake: BC",'Koch Snowflake: Dust')) +
  scale_x_discrete(labels = c('0.0-0.2','0.2-0.4','0.4-0.6','0.6-0.8','0.8~1.0')) + 
  ylim(c(-0.02,0.01))
...
ggsave(paste0(Season,"_Snow_mixing_comparison_TP.tiff"), plot = plot_figure, width = 18, height = 15, units = "cm", dpi = 300, limitsize = FALSE, compression = "lzw")
