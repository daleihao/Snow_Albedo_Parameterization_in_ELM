...
for i = 1:num
    ...
    if(year_i <=2000 | year_i>2010 | ~(month_i<6 & month_i>2))
        continue;
    end
...
fsno_spring = nansum(SnowCovers_filter.*area_tmp)/nansum(area_tmp);
delta_albedo_spring = nansum(deltaAlbedos_filter.*area_tmp)/nansum(area_tmp);
